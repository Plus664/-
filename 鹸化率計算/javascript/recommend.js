...
const get_option_adjustments = (recipe) => {
    ...
    if(recipe.option) {
        for(let i = 0; i < recipe.option.length; i++){
            const option_name = recipe.options[i].name;
            const option_data = window.OptionArray[option_name];

            if (option_data) {
                const percentage = Number(recipe.options[i].percentage) / 100;
                clean_adjustment += option_data.clean * percentage;
                foam_adjustment += option_data.foam * percentage;
                hard_adjustment += option_data.hard * percentage;
                stability_adjustment += option_data.stability * percentage;
            }
        }
    }
